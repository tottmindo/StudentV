...
import { ExternalEventRepository } from "../modules/events/externalEventRepository.js";

export type { CleaningWeek, CleaningWeekSwapRequest, CleaningWeekTask } from "../types/data.js";

class Data extends ExternalEventRepository {
...
async createSurvey(survey: any) {
  ...
  async getChatRooms(dormID: number, userID: number){
    try {
      const query = `SELECT c.chatID, c.createdAt, c.dormID, d.address, d.floor,
      ...
      return (rows as any[]).map(room => ({
        ...room,
        house: String(room.address ?? room.house ?? room.dormAddress ?? ""),
        floor: Number(room.floor ?? room.dormFloor),
