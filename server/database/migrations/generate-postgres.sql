...
CREATE TABLE dorms (
  ...
  CONSTRAINT dorms_address_number_check CHECK (address ~ '^[0-9]+$'),
...
CREATE TABLE cleaningtasktemplate (
  ...
  CONSTRAINT cleaningtasktemplate_one_scope CHECK (dormid IS NULL OR houseaddress IS NULL)
);
...
-- Global cleaning templates. NULL dormid and houseaddress make them available
-- to every dorm. House-wide and floor-specific templates are managed by admins.
