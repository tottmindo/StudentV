  ...
  <main class="mx-auto min-h-screen max-w-7xl space-y-6 px-4 py-6">
    ...
    <template v-else-if="activeSection === 'users'">
      ...
      <section class="flex flex-wrap items-center justify-between gap-5 rounded-2xl bg-surface p-6 shadow-lg dark:bg-surface-dark">
        <div class="max-w-3xl"><h2 class="text-xl font-bold">Cleaning week schedule</h2><p class="mt-1 text-sm opacity-75">Check every dorm and fill the configured schedule horizon. Existing valid assignments will not be changed.</p><p v-if="cleaningFeedback" class="mt-3 text-sm font-semibold" :class="cleaningFeedbackClass">{{ cleaningFeedback }}</p></div>
        <button :disabled="isGeneratingCleaning" class="rounded-lg bg-accent px-5 py-3 font-semibold text-white disabled:opacity-50" @click="generateCleaningSchedule">{{ isGeneratingCleaning ? 'Checking schedule…' : 'Generate cleaning weeks' }}</button>
      </section>
    </template>
...
async function generateCleaningSchedule() { isGeneratingCleaning.value = true; cleaningFeedback.value = ''; try { const response = await fetch(apiUrl('/api/auth/admin/cleaning-weeks/generate'), { method: 'POST', headers: headers() }); const data = await response.json(); if (!response.ok) throw new Error(data.error || 'Could not generate cleaning weeks.'); const summary = data.summary; cleaningFeedback.value = `${summary.weeksCreated} week(s) created, ${summary.weeksReassigned} invalid assignment(s) repaired, and ${summary.weeksUnchanged} existing week(s) left unchanged.`; cleaningFeedbackClass.value = 'text-green-600' } catch (error) { cleaningFeedback.value = error instanceof Error ? error.message : 'Could not generate cleaning weeks.'; cleaningFeedbackClass.value = 'text-red-500' } finally { isGeneratingCleaning.value = false } }
