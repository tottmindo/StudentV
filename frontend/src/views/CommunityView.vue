<template>
  <main class="min-h-screen bg-background-light px-4 py-6 text-text dark:bg-background-dark dark:text-text-dark sm:px-6 lg:px-8">
    <div class="mx-auto flex max-w-7xl flex-col gap-6">
      ...
      <template v-else>
        <header class="flex flex-wrap items-center justify-between gap-4"><h2 class="text-3xl font-bold">{{ t('communityHub.people') }}</h2><button class="rounded-xl border border-border-border px-4 py-2 font-bold" @click="activeSection = null">{{ t('common.back') }}</button></header>
    ...
    <div v-if="selectedResident" class="fixed inset-0 z-50 grid place-items-center bg-black/55 p-4" role="presentation" @click.self="closeResident">
      <section class="w-full max-w-md rounded-3xl bg-background p-6 text-text shadow-2xl dark:bg-surface-dark dark:text-text-dark" role="dialog" aria-modal="true" :aria-labelledby="`resident-${selectedResident.userID}`">
        <div class="flex items-start justify-between gap-4">
          <div class="flex min-w-0 items-center gap-4">
            ...
            <div class="min-w-0"><h2 :id="`resident-${selectedResident.userID}`" class="truncate text-2xl font-extrabold">{{ selectedResident.username }}</h2><p class="text-sm opacity-60">{{ t('communityHub.room', { room: selectedResident.roomID }) }}</p></div>
          </div>
          <button type="button" class="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-surface text-xl dark:bg-background-dark" :aria-label="t('common.close')" @click="closeResident">×</button>
        </div>
        <div class="mt-6 rounded-2xl bg-surface p-4 dark:bg-background-dark">
          ...
          <p v-if="selectedResident.bio" class="mt-2 whitespace-pre-line leading-7">{{ selectedResident.bio }}</p>
        ...
        <div v-if="!selectedResident.isCurrentUser" class="mt-6 grid gap-3 sm:grid-cols-2">
          <button type="button" class="rounded-xl bg-accent px-4 py-3 font-bold text-white disabled:opacity-45" :disabled="Boolean(residentActionBusy)" @click="startDirectChat">{{ residentActionBusy === 'chat' ? t('communityHub.openingChat') : t('communityHub.startChat') }}</button>
          <button type="button" class="rounded-xl border px-4 py-3 font-bold disabled:opacity-45" :class="selectedResident.isBlocked ? 'border-success/50 text-success' : 'border-border-border opacity-70'" :disabled="Boolean(residentActionBusy)" @click="toggleMessageBlock">
...
<style scoped>
.hub-card { @apply rounded-2xl bg-surface p-8 text-left shadow-lg transition hover:-translate-y-1 hover:shadow-xl dark:bg-surface-dark; }
.hub-card h2 { @apply mt-5 text-2xl font-bold; }
.hub-card p { @apply mt-2 opacity-75; }
</style>
