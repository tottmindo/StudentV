...
async function applyResidentImport() {
  ...
  const rows = residentImportRows.value.filter(row => selected.has(row.rowNumber) && row.roomID != null && row.dormID != null).map(row => ({ roomID: row.roomID!, dormID: row.dormID!, email: row.vacant ? 'vacant@invalid.local' : row.email, vacant: row.vacant }))
  if (!rows.length) return
  isResidentImportBusy.value = true; residentImportFeedback.value = ''
  try {
    const response = await fetch(apiUrl('/api/auth/admin/resident-import/apply'), { method: 'POST', headers: headers(), body: JSON.stringify({ rows }) })
    const data = await response.json(); if (!response.ok) throw new Error(data.error || t('adminMain.residentImportApplyError'))
    residentImportFeedback.value = t('adminMain.residentImportComplete', { succeeded: data.succeeded, failed: data.failed }); residentImportFeedbackClass.value = data.failed ? 'text-orange-600' : 'text-green-600'
    if (!data.failed) { residentImportRows.value = []; selectedResidentRows.value = []; await loadUsers() }
  } catch (error) { residentImportFeedback.value = error instanceof Error ? error.message : t('adminMain.residentImportApplyError'); residentImportFeedbackClass.value = 'text-red-500' }
  finally { isResidentImportBusy.value = false }
}
