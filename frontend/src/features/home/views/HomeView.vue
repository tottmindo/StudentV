<template>
  <main class="home-dashboard min-h-[calc(100dvh-4rem)] min-w-0 bg-background-light px-4 py-4 text-text dark:bg-background-dark dark:text-text-dark sm:px-6 lg:h-[calc(100dvh-4rem)] lg:min-h-0 lg:px-8 lg:py-3">
    <div class="mx-auto flex min-w-0 max-w-7xl flex-col space-y-4 lg:h-full">
      ...
      <template v-else>
        <section v-if="loadError" class="flex min-w-0 flex-wrap items-center justify-between gap-3 rounded-2xl border border-error/30 bg-error/10 p-4" role="alert"><div class="min-w-0"><p class="break-words font-bold">{{ t('home.partialError') }}</p><p class="break-words text-sm opacity-70">{{ t('home.partialErrorHelp') }}</p></div><button class="min-h-11 shrink-0 rounded-xl border border-error/40 px-4 py-2 text-sm font-bold" @click="requestDashboard">{{ t('common.retry') }}</button></section>

        <section v-if="alerts.length || userSurveys.length" aria-labelledby="attention-heading">
          <div class="mb-3 flex items-end justify-between gap-3"><div class="min-w-0"><p class="text-xs font-bold uppercase tracking-[.14em] text-accent">{{ t('home.upNext') }}</p><h2 id="attention-heading" class="break-words text-2xl font-bold">{{ t('home.needsAttention') }}</h2></div><span class="shrink-0 rounded-full bg-accent/10 px-3 py-1 text-sm font-bold text-accent">{{ attentionCount }}</span></div>
          <div class="grid gap-3 md:grid-cols-2">
            <article v-for="alert in alerts" :key="alert.id" class="relative flex min-h-32 min-w-0 items-start gap-4 rounded-2xl border border-border-border bg-background p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-accent dark:bg-surface-dark">
              <span class="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-warning/15 text-xl">!</span>
              <router-link :to="alert.route || '/home'" class="min-w-0 flex-1 pr-8 after:absolute after:inset-0 after:content-['']"><strong class="block break-words text-lg">{{ alert.title }}</strong><span class="mt-1 block break-words text-sm opacity-70">{{ alert.description }}</span><span class="mt-3 block break-words text-sm font-bold text-accent">{{ alert.actionLabel || t('home.open') }} →</span></router-link>
            ...
            <article v-if="userSurveys.length" class="relative flex min-h-32 min-w-0 items-start gap-4 rounded-2xl border border-border-border bg-background p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-accent dark:bg-surface-dark">
              <span class="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-accent/10 text-xl">≡</span>
              <button type="button" class="survey-answer-trigger min-w-0 flex-1 text-left after:absolute after:inset-0 after:content-['']" @click="openSurvey(userSurveys[0].eID)"><strong class="block break-words text-lg">{{ t('notifications.pendingSurveys') }}</strong><span class="mt-1 block break-words text-sm opacity-70">{{ t('notifications.pendingSurveyCount', { count: userSurveys.length }) }}</span><span class="mt-3 block break-words text-sm font-bold text-accent">{{ t('notifications.answerSurveys') }} →</span></button>
            </article>
          </div>
        </section>
        <section v-else class="flex min-w-0 items-center gap-4 rounded-2xl border border-success/20 bg-success/10 p-5"><span class="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-success text-xl font-bold text-white">✓</span><div class="min-w-0"><h2 class="break-words font-bold">{{ t('notifications.caughtUp') }}</h2><p class="break-words text-sm opacity-70">{{ t('home.caughtUpHelp') }}</p></div></section>

        <div class="grid gap-6 lg:grid-cols-[minmax(0,1.35fr)_minmax(19rem,.65fr)]">
          <section class="min-w-0 rounded-3xl bg-background p-5 shadow-sm dark:bg-surface-dark sm:p-6" aria-labelledby="water-heading">
            <div class="flex flex-wrap items-start justify-between gap-3"><div class="min-w-0"><p class="text-xs font-bold uppercase tracking-[.14em] text-accent">{{ t('topbar.yourFloor') }}</p><h2 id="water-heading" class="break-words text-2xl font-bold">{{ t('home.waterUse') }}</h2><p class="mt-1 break-words text-sm opacity-65">{{ t('home.waterHelp') }}</p></div><router-link to="/stats" class="inline-flex min-h-11 shrink-0 items-center rounded-xl border border-border-border px-4 py-2 text-sm font-bold hover:border-accent hover:text-accent">{{ t('home.viewInsights') }} →</router-link></div>
          ...
          <section class="min-w-0 rounded-3xl bg-background p-5 shadow-sm dark:bg-surface-dark sm:p-6" aria-labelledby="events-heading">
            <div class="flex items-center justify-between gap-3"><div class="min-w-0"><p class="text-xs font-bold uppercase tracking-[.14em] text-accent">{{ t('nav.community') }}</p><h2 id="events-heading" class="break-words text-2xl font-bold">{{ t('home.comingUp') }}</h2></div><router-link to="/events" class="inline-flex min-h-11 shrink-0 items-center text-right text-sm font-bold text-accent">{{ t('home.allEvents') }} →</router-link></div>
        ...
        <section v-if="isAdmin" class="min-w-0 rounded-3xl border border-accent/25 bg-accent/5 p-5 sm:p-6" aria-labelledby="admin-heading"><div class="flex flex-wrap items-end justify-between gap-3"><div class="min-w-0"><p class="text-xs font-bold uppercase tracking-[.14em] text-accent">{{ t('topbar.administrator') }}</p><h2 id="admin-heading" class="break-words text-2xl font-bold">{{ t('home.managementWorkspace') }}</h2><p class="mt-1 break-words text-sm opacity-65">{{ t('home.managementHelp') }}</p></div><router-link to="/admin" class="shrink-0 rounded-xl bg-accent px-4 py-2.5 text-sm font-bold text-white">{{ t('home.openAdmin') }}</router-link></div><div class="mt-5 grid gap-3 sm:grid-cols-3"><router-link v-for="item in adminShortcuts" :key="item.to" :to="item.to" class="min-w-0 rounded-2xl bg-background p-4 font-bold shadow-sm hover:text-accent dark:bg-surface-dark"><span class="mr-2">{{ item.icon }}</span><span class="break-words">{{ item.label }} →</span></router-link></div></section>
      </template>
    </div>

    <ModalComponent v-model="showEventModal"><div v-if="selectedEvent" class="min-w-0 space-y-4 p-2 sm:p-4"><div><p v-if="selectedEvent.type" class="break-words text-xs font-bold uppercase tracking-wider text-accent">{{ selectedEvent.type }}</p><h2 class="mt-1 break-words pr-8 text-2xl font-bold sm:pr-12">{{ selectedEvent.title }}</h2></div><p class="break-words font-semibold">{{ formatEventDateRange(selectedEvent.startDate, selectedEvent.endDate) || selectedEvent.time || t('home.dateTba') }}</p><p class="whitespace-pre-line break-words opacity-75">{{ selectedEvent.description || t('home.noEventInfo') }}</p><a v-if="selectedEvent.externalUrl" :href="selectedEvent.externalUrl" target="_blank" rel="noopener noreferrer" class="inline-flex max-w-full break-words rounded-xl bg-accent px-4 py-2.5 font-bold text-white">{{ t('home.openEventPage') }}</a><router-link v-else to="/events" class="inline-flex max-w-full break-words rounded-xl bg-accent px-4 py-2.5 font-bold text-white" @click="showEventModal = false">{{ t('home.openCalendar') }}</router-link></div></ModalComponent>
  </main>
</template>

<script setup lang="ts">
import { computed, onBeforeUnmount, onMounted, ref } from 'vue'
import FloorWaterChart from '@/features/water/components/FloorWaterChart.vue'
